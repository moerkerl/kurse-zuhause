...
const kurstypenBeispiele = [
  'Sprachkurse',
  'Nachhilfe (Mathe, Physik, etc.)',
  'Musikunterricht',
...
export default function MultiStepForm() {
  ...
  const handleNext = (e?: React.FormEvent) => {
    ...
    if (step === 2 && !form.kurstyp) return setError('Bitte wählen Sie einen Kurstyp.')
      ...
      {step === 2 && (
        <form onSubmit={handleNext}>
          ...
          <input
            ...
            placeholder="Kurstyp oder Fach eingeben..."
          />
          
          <div className="mb-4">
            <p className="text-sm text-gray-600 mb-3">Oder wählen Sie aus beliebten Optionen:</p>
            <div className="grid grid-cols-1 gap-2">
              {kurstypenBeispiele.map(kurstyp => (
                <button
                  key={kurstyp}
                  type="button"
                  className={`border-2 rounded-lg px-4 py-3 text-left transition-all cursor-pointer ${
                    form.kurstyp === kurstyp
                      ? 'border-[#047857] bg-blue-50 text-[#010583]'
                      : 'border-gray-300 hover:border-blue-400 text-gray-700'
                  }`}
                  onClick={() => setForm({ ...form, kurstyp })}
                >
                  {kurstyp}
                </button>
              ))}
      ...
      {step === 4 && (
        <form onSubmit={handleSubmit}>
          ...
          <p className="text-gray-600 mb-6">
            Damit wir Ihnen die passende Auswahl an Kursleitern in der Schweiz zusenden können.
          </p>
